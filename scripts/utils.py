...
from scripts.losses import fusion_prompt_loss,align_loss
...
multDegOfOne_prompt_path = "/home/zyq/Text-IF/dataset/EMS_new/IR_Lowcon_VI_LowLight/train/text.txt"
assert os.path.exists(multDegOfOne_prompt_path), "text prompt root: {} does not exist.".format(multDegOfOne_prompt_path)
with open(multDegOfOne_prompt_path, 'r', encoding='utf-8') as file:
        multDegOfOne_lines = file.readlines()
...
def get_multDegOfOne_prompt():
    random_line =random.choice(multDegOfOne_lines)
    random_line = random_line.strip()
    return random_line


def train_two_epoch(model, model_clip, optimizer, lr_scheduler, data_loader, device, epoch,type="teacher"):
    model.train()
    model_clip.eval()
    loss_function_prompt = fusion_prompt_loss()
    loss_function_align = align_loss()
    ...
    for step, data in enumerate(data_loader):
        ...
        # default type degradation in vis image
            ...
            elif task[index] == "multDegOfOne":
                text_line.append(get_multDegOfOne_prompt())
            else:
        ...
        I_fused, F_unified, F_teacher = model(I_A, I_B, text,type,device=device)

        if type=="student":
            loss, loss_ssim, loss_max, loss_color, loss_text = loss_function_align(I_A_gt, I_B_gt, I_fused, task, F_unified, F_teacher)
        else:
...
def train_one_epoch(model, model_clip, optimizer, lr_scheduler, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        I_fused,_,_ = model(I_A, I_B, text)
    ...
    for step, data in enumerate(data_loader):
        ...
        for index in range(len(task)):
            if task[index] == "low_light":
                ...
                text_line.append("This is a visible light and infrared image fusion task. Visible light images may have overexposure or low light degradation, while infrared images may have noise or low contrast degradation.")
            else:
                text_line.append("This is unknown to the image fusion task.")
