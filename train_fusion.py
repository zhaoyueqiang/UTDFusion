...
# lsof -i:6006
import clip
from data.prompt_dataset import PromptDataSet,PromptDataSet_mutli

from model.Text_MF_model import Text_MF as create_model
from scripts.utils import read_data, train_one_epoch, evaluate, create_lr_scheduler,train_two_epoch
import datetime
import warnings
...
def main(args):
    ...
    print("Loading IVF Fusion and multDegOfOne Task!")
    if args.multDegOfOne_path is not None:
        train_multDegOfOne_path_list, val_multDegOfOne_path_list = read_data(args.multDegOfOne_path)
    else:
        train_multDegOfOne_path_list = val_multDegOfOne_path_list = None
    ...
    train2_dataset=PromptDataSet_mutli(train_multDegOfOne_path_list=train_multDegOfOne_path_list,
                                       val_multDegOfOne_path_list=val_multDegOfOne_path_list,
                                       phase="train", 
                                    transform=data_transform["train"])
    val2_dataset=PromptDataSet_mutli(train_multDegOfOne_path_list=train_multDegOfOne_path_list,
                                       val_multDegOfOne_path_list=val_multDegOfOne_path_list,
                                       phase="val", 
                                    transform=data_transform["val"])
    ...
    train2_loader = torch.utils.data.DataLoader(train2_dataset,
                                               batch_size=batch_size,
                                               shuffle=True,
                                               pin_memory=True,
                                               num_workers=nw,
                                               collate_fn=train2_dataset.collate_fn)
    val2_loader = torch.utils.data.DataLoader(val2_dataset,
                                             batch_size=1,
                                             shuffle=False,
                                             pin_memory=True,
                                             num_workers=nw,
                                             collate_fn=val2_dataset.collate_fn)
    ...
    lr_scheduler = create_lr_scheduler(optimizer, len(train_loader)+len(train2_loader), args.epochs+second_epochs, warmup=True)
    ...
    for epoch in range(start_epoch, args.epochs+second_epochs):
        ...
        # train
            ...
            train_loss, train_ssim_loss, train_max_loss, train_color_loss, train_text_loss, lr = train_two_epoch(model=model,
                                                model_clip=model_clip,
                                                    optimizer=optimizer,
                                                    data_loader=train2_loader,
                                                    lr_scheduler=lr_scheduler,
                                                    device=device,
                                                    epoch=epoch,
                                                    type="student")
        ...
        if epoch % args.val_every_epcho == 0 and epoch != 0:
            ...
            if val_loss < best_val_loss and epoch > args.epochs:
                if args.use_dp == True:
            ...
            if args.use_dp == True:
                    ...
                    save_file = {"model": model.state_dict(),
                                 ...
                                 "epoch": epoch,
                                 "args": args}
            if epoch<args.epochs and epoch>= args.epochs-10:
                if args.use_dp == True:
                    save_file = {"model": model.module.state_dict(),
                                 "optimizer": optimizer.state_dict(),
                                 "lr_scheduler": lr_scheduler.state_dict(),
                    ...
                    save_file = {"model": model.state_dict(),
                                 ...
                                 "epoch": epoch,
                                 "args": args}
                torch.save(save_file, file_weights_path + "/" + "checkpoint_lastest_stage1.pth")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--multDegOfOne_path', type=str, default="./dataset/EMS_lite/multDegOfOne")
    ...
    parser.add_argument('--gpu_id', default='0,1', help='device id (i.e. 0, 1, 2 or 3)')
